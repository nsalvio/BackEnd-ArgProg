...
public class SkillsController {
    ...
    @PostMapping("/new") //agrega skills mediante la búsqueda de Persona por id
    ...
    @PutMapping ("/edit/{id}") //edita el contenido de Persona
    public Skills  editSkill( 
            @PathVariable Long idPersona,            
            @PathVariable Long id,
                                 @RequestBody Skills skill
            ) {
 
    Skills skillGuardado = skillsService.findSkill(id);
   
    if(skillGuardado == null)
        return null;
            
    skillGuardado.setNombreSkill(skill.getNombreSkill());
    skillGuardado.setPorcentaje(skill.getPorcentaje());
   
    skillsService.saveSkill(skillGuardado);
    
    skillGuardado.setPersona(null);
   
    return skillGuardado;
